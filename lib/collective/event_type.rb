module Collective
  class EventType < Base
    ...
    def self.all(args = {})
      ...
      event_types = create_api_objects(data, Collective::EventType, :event_types)
      sub_event_types = create_api_objects(data, Collective::EventType, :sub_event_types)
      event_types += sub_event_types
      event_types.each { |e| e.type = "SiteEventType" }

      data = Collective::Base.streets_events_types_get(args)
      street_events = create_api_objects(data, Collective::EventType, :event_types)
    ...
    def parent
      if @json[:event_type]
        parent = Collective::EventType.new(@json[:event_type])
        parent.type = @type
        parent
      end
    end
